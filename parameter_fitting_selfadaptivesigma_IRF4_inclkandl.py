...
class IRF4(object):
    ...
    def __init__(self, mu_r, sigma_r, k_r, lambda_r, CD40):
        self.mu_r = mu_r
    ...
    def plot(self, name='test'):
        ...
        fig.savefig("IRF4_inclkandl_{}.png".format(name.capitalize()))
        plt.close(fig)
...
def fitness(ind):
    ...
    mu_r, sigma_r, k_r, lambda_r, CD40 = ind
    ...
    if (beta ** 2 > 3) and (beta ** 3 + (beta ** 2 - 3) ** (3/2) - 9 * beta / 2 > - 27/2 * p) and \
            ...
            (mu_r > 0) and (sigma_r > 0) and (k_r > 0 and k_r < 20) and (lambda_r > 0 and lambda_r < 20) and (CD40 >= 0):
...
def run_evolutionary_algo(name_run, pop_size, num_variables, num_gen, tournament_size, GC_data, PC_data, CB_sep_data, CC_sep_data):
    ...
    with open("IRF4data_{}.csv".format(name_run), "w") as file:
        writer = csv.DictWriter(file, delimiter=',', fieldnames = ["generation", "fitness", 'mu_r', 'sigma_r', 'k_r', 'lambda_r', 'CD40'])
        writer.writeheader()
    ...
    for gen in range(num_gen):
        ...
        k_list[gen * len(fitnesses): (gen + 1) * len(fitnesses)] = np.array(population)[:, 2]
        lambda_list[gen * len(fitnesses): (gen + 1) * len(fitnesses)] = np.array(population)[:, 3]
        CD40_list[gen * len(fitnesses): (gen + 1) * len(fitnesses)] = np.array(population)[:, 4]
        ...
        with open("IRF4data_{}.csv".format(name_run), "a") as file:
            writer = csv.writer(file, delimiter=',')
            writer.writerow([gen, best_fit_gen, *best_sol_current[:number_of_variables_to_fit]])


    results = pd.DataFrame(data=np.array([sigma_list, mu_list, k_list, lambda_list, CD40_list, fitness_list]).T,
                           columns=['sigma', 'mu', 'k', 'lambda', 'CD40', 'fitness'])
...
if __name__ == '__main__':
    ...
    print("The fitness of our solution: ", fitness([best_solution.mu_r, best_solution.sigma_r,
                                                    best_solution.k_r, best_solution.lambda_r, best_solution.CD40, 0, 0, 0, 0, 0]))
    ...
    sol_of_martinez = IRF4(mu_r, sigma_r, k_r, lambda_r, CD40)
    ...
    print("The fitness of the martinez solution: ", fitness([mu_r, sigma_r, k_r, lambda_r, CD40, 0, 0, 0, 0, 0]))
