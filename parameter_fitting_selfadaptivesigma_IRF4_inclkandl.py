...
def fitness(ind):
    ...
    if (beta ** 2 > 3) and (beta ** 3 + (beta ** 2 - 3) ** (3 / 2) - 9 * beta / 2 > - 27 / 2 * p) and \
            ...
            (mu_r > 0) and (sigma_r > 0) and (k_r > 0 and k_r < 20) and (lambda_r > 0 and lambda_r < 20) and (
            CD40 >= 0):
...
def run_evolutionary_algo(name_run, pop_size, num_variables, num_gen, tournament_size, GC_data, PC_data, CB_sep_data,
    ...
    with open("IRF4data_{}.csv".format(name_run), "w") as file:
        writer = csv.DictWriter(file, delimiter=',',
                                fieldnames=["generation", "fitness", 'mu_r', 'sigma_r', 'k_r', 'lambda_r', 'CD40'])
        writer.writeheader()
...
if __name__ == '__main__':
    ...
    print("The fitness of our solution: ", fitness([best_solution.mu_r, best_solution.sigma_r,
                                                    best_solution.k_r, best_solution.lambda_r, best_solution.CD40, 0, 0,
                                                    0, 0, 0]))
